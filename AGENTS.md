...
- The `maintenance` app is technical tooling only: no models, no migrations. It hosts `export_migration_bundle`, `import_migration_bundle` and `verify_migration_bundle`, whose shared logic lives in `maintenance/database_transfer.py`; management commands must only parse arguments, print output and call those services.
...
- Import is only allowed into a PostgreSQL database whose transferable tables are empty. The tools must never flush, delete existing rows, or silently overwrite a non-empty `MEDIA_ROOT`; rows created by data migrations (`references.ActStatus`, `references.TaskStatus`) are cleared by the operator, not by the command.
- Every bundle carries SHA-256 checksums for `data.json` and each media file plus per-model counts, max PKs and deterministic hashes. Import must fully validate the bundle before writing anything, and a transfer is only considered done once `verify_migration_bundle` passes.
...
- A migration bundle contains real production data (users, acts, attachments) and must never be committed to Git; bundle directories, SQLite backups and transfer reports stay in `.gitignore`.
- Successful export/import/verify runs prove the tooling works — they do not mean the production transfer has been performed or that PostgreSQL is deployed.
