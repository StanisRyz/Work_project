...
- RT-2 deliberately excludes `EventSource` and any frontend JavaScript, toasts, bell updates, partial endpoints, automatic act/task refresh, act subscriptions, a sync endpoint, fallback polling, `BroadcastChannel`, WebSocket/Channels, production reverse proxy and HTTPS, event storage in PostgreSQL, and a transactional outbox. See `docs/realtime.md`.
