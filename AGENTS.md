...
- RT-4 deliberately excludes extra SSE connections, browser subscriptions to `act:<id>`, `/realtime/sync/`, fallback polling, `BroadcastChannel` and leader-tab election, WebSocket/Channels, React, event replay or storage, production reverse proxy and HTTPS, load testing and Redis monitoring. See `docs/realtime.md`.
