...
`?next=`. Пользовательский интерфейс состоит только из разделов «Акты» и
«Задачи»; управление пользователями, ролями и справочниками выполняется через
Django Admin (`/admin/`). Действия по маршруту подтверждаются модальным окном
