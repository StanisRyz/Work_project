...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--allow-missing-media',
            action='store_true',
            help='Не прерывать экспорт при отсутствующих файлах вложений (будут перечислены в manifest).',
        )

    def handle(self, *args, **options):
        try:
            manifest = export_bundle(
                options['output'], allow_missing_media=options['allow_missing_media']
            )
        ...
        if manifest['missing_media']:
            self.stdout.write(
                self.style.WARNING(
                    f'Отсутствующие файлы вложений — {len(manifest["missing_media"])}:'
                )
            )
