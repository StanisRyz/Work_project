...
Обязательные справочники — это полный набор статусов актов
(`CREATED_OTK`, `KO_REVIEW`, `TO_ANALYSIS`, `OTK_REVIEW`, `ACTIONS_ASSIGNED`,
`ARCHIVED`, `CLOSED`, `CANCELLED`, тот же список, что использует
`acts.models.get_act_status()`) и статусов задач (`IN_PROGRESS`, `COMPLETED`).
`seed_references` создаёт ровно этот набор.
...
Для каждого года, в котором существуют акты с каноническим номером
`АОК-YYYY-NNN`, команда вычисляет фактически выданный максимальный номер (одним
запросом, только колонка `number`, без роста числа запросов от количества
актов или лет) и сравнивает его со строкой `ActNumberSequence.last_value` за
тот же год:

| Ситуация | Результат |
| --- | --- |
| Актов и строк счётчика нет | `PASS` |
| Строка счётчика есть, актов за год нет, `last_value >= 0` | `PASS` |
| Акты за год есть, строки счётчика нет | `BLOCKING` |
...
**только читает**: она никогда не создаёт отсутствующую строку и никогда не
поднимает `last_value` сама. Сообщение называет только год и агрегированные
значения — конкретный номер акта в нём никогда не печатается.

**Ручное исправление** (эта команда его не выполняет): через `manage.py shell`
или административную консоль под учётной записью с правом записи в базу,
согласовав действие с ответственным за эксплуатацию:

```python
from acts.models import ActNumberSequence
# Поднять счётчик минимум до фактического максимума за год, полученного из
# отчёта check_fresh_bootstrap, и никогда — вручную угаданное число.
...
Разрушительная или автоматическая management-команда для этого исправления в
проекте намеренно отсутствует: несогласованное изменение счётчика может выдать
уже занятый номер.
...
версия PostgreSQL, миграции, справочники, `ActNumberSequence`, Redis (с
настоящим PING при включённом real-time), static/media, demo reset, email,
подтверждение резервного копирования. Каждый логический раздел встречается в
отчёте ровно один раз: команда переиспользует базовые проверки
`check_fresh_bootstrap`, а real-time, static/media, demo reset и email
проверяет сама, более подробно (настоящий PING, сравнение `MEDIA_ROOT` и
`STATIC_ROOT`), не дублируя те же имена проверок. При наличии `BLOCKING` код
возврата ненулевой.
